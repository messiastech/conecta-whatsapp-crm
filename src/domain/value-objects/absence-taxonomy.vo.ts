...
export const AbsenceAnalysisSchema = z.object({
  ...
  urgency: UrgencyEnum,
  suggested_reply: z.string().max(400)
});
...
export const CategoryLabels: Record<AbsenceCategory, { label: string; color: string; description: string }> = {
  TRABALHO: { label: 'Trabalho / Plantão', color: '#3B82F6', description: 'Escala de serviço, hora extra ou trabalho' },
  ...
  VIAGEM: { label: 'Viagem / Férias', color: '#10B981', description: 'Fora da cidade ou em trânsito' },
  ...
  FALTA_INFORMACAO: { label: 'Falta de Informação', color: '#EC4899', description: 'Não sabia o local, horário ou não recebeu convite' },
