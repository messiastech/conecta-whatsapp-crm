...
import { MessageSquare, Lock, Mail, User, ArrowRight, Sparkles, Building } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="flex items-center justify-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">Conecta CRM</h1>
            <p className="text-xs text-emerald-400 font-medium tracking-wide uppercase flex items-center gap-1">
              <Sparkles className="w-3 h-3" /> SaaS Multi-Tenant & IA
            </p>
          </div>
        </div>

        <h2 className="mt-6 text-center text-xl font-medium text-slate-300">
          {isSignUp ? 'Crie sua conta e seu Workspace' : 'Entre no seu Workspace'}
        </h2>
      </div>
        ...
        <div className="bg-slate-800/90 backdrop-blur-md border border-slate-700/60 py-8 px-6 shadow-2xl rounded-2xl sm:px-10">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="pastor@suacomunidade.org"
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-900/60 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full mt-2 py-3 px-4 bg-emerald-600 hover:bg-emerald-500 text-white font-medium rounded-xl shadow-lg shadow-emerald-600/30 flex items-center justify-center gap-2 transition-all disabled:opacity-50 text-sm cursor-pointer"
            >
              {loading ? (
                'Processando...'
              ) : isSignUp ? (
